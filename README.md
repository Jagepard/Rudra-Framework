...
```
>💡 Tip: The framework's Query Builder (Rudra-Model) automatically reads these settings based on the active environment. Simply uncomment the dsn line for your preferred database engine (MySQL, PostgreSQL, or SQLite).
...
```
#### 🖥️ Console Environment

For CLI commands (like `php rudra migrate`), the framework cannot auto-detect the environment as it does in `index.php`. Instead, it relies on the `app_env.php` file to determine which configuration file to load (e.g., `'local'` → `config/setting.local.yml`).
...
```
> ℹ️ **Note:** If `app_env.php` is missing (e.g., if you cloned the repository directly instead of using `composer create-project`), the framework defaults to `'local'` for CLI commands. This is safe for local development but should not be relied upon in production — always create the file manually or set the `APP_ENV` server variable explicitly.
